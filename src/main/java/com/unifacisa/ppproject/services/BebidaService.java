...
public class BebidaService extends BasicService<Bebida, BebidaRepository> {
	...
	public Bebida post(Bebida element) {
		...
		List<Long> ingredienteIds = ingredientes.stream().map(e -> e.getIngrediente().getId()).collect(Collectors.toList());
		...
		element.setPreco(ingredientes.stream().mapToDouble(e -> ingredientesMap.get(e.getIngrediente().getId()).getPreco() * e.getQtd() ).sum() );
